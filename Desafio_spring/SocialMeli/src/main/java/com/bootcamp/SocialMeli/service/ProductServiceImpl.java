...
/*        for (int i = 0; i < repositoryProducts.getPosts().size(); i++) {
            if (repositoryProducts.getPosts().get(i).getPostId()== newPost.getPostId()) {
                throw new DuplicatesFoundException(newPost.getPostId());
            }
        }*/
        //formatting date
        ...
        if (repositoryUsers.userExists(userId)){
            ...
            if (order.equalsIgnoreCase("date_asc")){
                for (int i = 0;i<followedIDs.size();i++){
                    ...
                    Collections.sort(postResponseDTO.getPosts() , Comparator.comparing(PostResponseDTO::getDatew));
                }
            }
